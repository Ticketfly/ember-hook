...
test('it returns the second value if the first has a test environment', function(assert) {
  const result = returnWhenTesting({ environment: 'test' }, 'foo');
  assert.equal(result, 'foo');
});

test('it returns nothing if the first value does not contain test environment', function(assert) {
